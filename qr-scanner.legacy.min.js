...
MediaStream))return!1;c=a.$video.srcObject}else c=(yield a._getCameraStream()).stream;return"torch"in c.getVideoTracks()[0].getSettings()}catch(d){return!1}finally{c&&c!==a.$video.srcObject&&(console.warn("Call hasFlash after successfully starting the scanner to avoid creating a temporary video stream"),b._stopVideoStream(c))}})}isFlashOn(){return this._flashOn}toggleFlash(){const a=this;return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){a._flashOn?yield a.turnFlashOff():yield a.turnFlashOn()})}turnFlashOn(){const a=
...
c._preferredCamera&&(c._preferredCamera=a,yield c._restartVideoStream())})}static scanImage(a,c,d,f,g=!1,h=!1){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){let l,m=!1;c&&("scanRegion"in c||"qrEngine"in c||"canvas"in c||"disallowCanvasResizing"in c||"alsoTryWithoutScanRegion"in c||"returnDetailedScanResult"in c)?(l=c.scanRegion,d=c.qrEngine,f=c.canvas,g=c.disallowCanvasResizing||!1,h=c.alsoTryWithoutScanRegion||!1,m=!0):c||d||f||g||h?console.warn("You're using a deprecated api for scanImage which will be removed in the future."):
console.warn("Note that the return type of scanImage will change in the future. To already switch to the new api today, you can pass returnDetailedScanResult: true.");let r=!!d;try{let n,q;[d,n]=yield Promise.all([d||b.createQrEngine(b.formats),b._loadImage(a)]);[f,q]=b._drawToCanvas(n,l,f,g);let p;if(d instanceof Worker){let k=d;r||b._postWorkerMessageSync(k,"inversionMode","both");p=yield new Promise((t,z)=>{let x,v,w,y=-1;v=u=>{u.data.id===y&&(k.removeEventListener("message",v),k.removeEventListener("error",
...
this.$video,c=a.videoWidth,d=a.videoHeight,f=a.offsetWidth,g=a.offsetHeight,h=a.offsetLeft,l=a.offsetTop,m=window.getComputedStyle(a),r=m.objectFit,n=c/d,q=f/g;switch(r){case "none":var p=c;var k=d;break;case "fill":p=f;k=g;break;default:("cover"===r?n>q:n<q)?(k=g,p=k*n):(p=f,k=p/n),"scale-down"===r&&(p=Math.min(p,c),k=Math.min(k,d))}var [t,z]=m.objectPosition.split(" ").map((v,w)=>{const y=parseFloat(v);return v.endsWith("%")?(w?g-k:f-p)*y/100:y});m=this._scanRegion.width||c;q=this._scanRegion.height||
d;r=this._scanRegion.x||0;var x=this._scanRegion.y||0;n=this.$overlay.style;n.width=`${m/c*p}px`;n.height=`${q/d*k}px`;n.top=`${l+z+x/d*k}px`;d=/scaleX\(-1\)/.test(a.style.transform);n.left=`${h+(d?f-t-p:t)+(d?c-r-m:r)/c*p}px`;n.transform=a.style.transform}})}static _convertPoints(a,c){if(!c)return a;let d=c.x||0,f=c.y||0,g=c.width&&c.downScaledWidth?c.width/c.downScaledWidth:1;c=c.height&&c.downScaledHeight?c.height/c.downScaledHeight:1;for(let h of a)h.x=h.x*g+d,h.y=h.y*c+f;return a}_scanFrame(){!this._active||
...
"environment":/front|user|face/i.test(a.label)?"user":null:null}static _drawToCanvas(a,c,d,f=!1){d=d||document.createElement("canvas");let g=c&&c.x?c.x:0,h=c&&c.y?c.y:0,l=c&&c.width?c.width:a.videoWidth||a.width,m=c&&c.height?c.height:a.videoHeight||a.height;f||(f=c&&c.downScaledWidth?c.downScaledWidth:l,c=c&&c.downScaledHeight?c.downScaledHeight:m,d.width!==f&&(d.width=f),d.height!==c&&(d.height=c));c=d.getContext("2d",{alpha:!1});c.imageSmoothingEnabled=!1;c.drawImage(a,g,h,l,m,0,0,d.width,d.height);
